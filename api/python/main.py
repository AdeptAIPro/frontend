...
@app.get("/api/talent-sources")
async def get_talent_sources():
    """Get available talent sources"""
    try:
        ...
        logger.error(f"Error fetching talent sources: {e}")
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"Error fetching talent sources: {str(e)}"
        )
