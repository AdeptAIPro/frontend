...
import { Bookmark, Phone, Star, Check, ShieldCheck, Mail } from "lucide-react";
...
const CandidateCard: React.FC<CandidateCardProps> = ({
  ...
  const getMatchScoreColor = (score: number) => {
    ...
    if (score >= 70) return "bg-blue-100 text-blue-800 hover:bg-blue-200";
      ...
      <CardContent className="p-6">
        <div className="flex flex-col md:flex-row gap-4">
          ...
          <div className="flex flex-row md:flex-col items-center gap-4">
            <Avatar className="h-20 w-20 border-2 border-adept/20">
              <AvatarImage src={candidate.avatar} alt={candidate.name} />
              <AvatarFallback className="bg-adept/10 text-adept">
            ...
            <div className="flex justify-between items-start">
              ...
              {candidate.contact && (
                <Button
                  variant="ghost"
                  size="sm"
                  className="text-muted-foreground hover:text-adept"
                  onClick={() => window.location.href = `mailto:${candidate.contact?.email}`}
                >
            ...
            <div className="mt-3 grid grid-cols-3 gap-2 text-xs">
              {candidate.skillMatch && (
                <div className="bg-slate-50 p-2 rounded">
                  ...
                  <div className="text-adept">{candidate.skillMatch}%</div>
                </div>
              )}
              {candidate.experienceMatch && (
                <div className="bg-slate-50 p-2 rounded">
                  <div className="font-medium">Experience</div>
                  <div className="text-adept">{candidate.experienceMatch}%</div>
                </div>
              )}
              {candidate.culturalFitScore && (
                <div className="bg-slate-50 p-2 rounded">
                  <div className="font-medium">Cultural Fit</div>
                  <div className="text-adept">{candidate.culturalFitScore}%</div>
                </div>
              )}
            </div>
